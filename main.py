...
# ---------------------- RTC (RandomForest + OOB / GPU 가속 옵션) ----------------------
# backend 옵션:
#  - 'sklearn': scikit-learn RandomForest + OOB (정확 재현, CPU)
#  - 'cuml_cv': RAPIDS cuML RandomForest를 사용, K-fold 교차검증으로 OOB 근사 (GPU)
...
def compute_pS0_stat(
    ...
    if backend == 'sklearn':
        ...
        try:
            ...
            raise RuntimeError("cuML이 설치되어 있어야 backend='cuml_cv'를 사용할 수 있습니다. Conda에서 RAPIDS를 설치하세요.") from e
        # numpy -> cupy
        ...
        for tr_idx, te_idx in kf.split(X_np):
            # 학습: 훈련 폴드에서
            X_tr = cp.asarray(X_np[tr_idx])
            y_tr = cp.asarray(y_np[tr_idx])
            model = cuRF(
                n_estimators=n_estimators,
                max_features=sqrt_int(d),
                bootstrap=True,
            ...
            model.fit(X_tr, y_tr)
            # 예측: 검증 폴드 중 S0 영역에 해당하는 인덱스만 선택
            te_S0_mask = np.isin(te_idx, idx_S0)
            te_S0_idx = te_idx[te_S0_mask]
            if len(te_S0_idx) == 0:
                continue
            X_te = cp.asarray(X_np[te_S0_idx])
            proba = model.predict_proba(X_te)  # cupy 배열
            p0 = cp.asnumpy(proba)[:, 0]
            p0_vals.append(p0)
        if not p0_vals:
            # 폴드 분할에 따라 S0 샘플이 검증에 하나도 안 걸렸다면 전체에서 한 번 예측
...
def train_rl_policy(
    ...
    if hasattr(torch, 'compile'):
        try:
            policy = torch.compile(policy)  # PyTorch 2.x 최적화
        except Exception:
            pass
    ...
    for ep in range(cfg.episodes):
        ...
        for t in range(1, scen.T + 1):
            ...
            for i, w in enumerate(actions):
                ...
                pS0 = compute_pS0_stat(Xrf, yrf, np.arange(len(S0_ref)), d=scen.d, n_estimators=200, seed=rng.randint(1_000_000), backend=rf_backend)
                ms_list.append(pS0)
                calib = calib_map[w]
    ...
    for t in range(1, T + 1):
        ...
        with torch.no_grad():
            ...
            if isinstance(policy, nn.DataParallel):
                logits = logits.module if hasattr(logits, 'module') else logits
...
def main():
    ...
    parser.add_argument('--rf_backend', type=str, default='sklearn', choices=['sklearn','cuml_cv'], help="RandomForest 백엔드: 'sklearn'은 CPU OOB(정확 재현), 'cuml_cv'는 cuML+K-fold로 OOB 근사(GPU)")
    ...
    calib_map: Dict[int, WindowCalib] = {}
    for w in action_set:
        calib = estimate_CL_for_window(S0_ref, scen.d, window=w, n_boot=args.n_boot, n_estimators=300, seed=rng.randint(1_000_000), backend=args.rf_backend)
    ...
    print("\n" + "="*60)
    print(f"시뮬레이션 종료. 총 소요 시간: {total_duration}")
